// 문서 자동화가 매주 실행될 요일·시간을 사용자가 직접 설정한다.
...
function extractHour(time = "21:00") {
  ...
  return hour ? hour.padStart(2, "0") : "21";
}
...
export default function AutomationScheduleSettings() {
  ...
  const isDirty = dayOfWeek !== savedSchedule.dayOfWeek || hour !== extractHour(savedSchedule.time);
  ...
  const handleSubmit = async (event) => {
    ...
    // 요청 시점의 요일·시간을 스냅샷으로 고정해, 응답을 기다리는 동안 다른 값으로 다시 제출돼도
    // 이번 요청은 항상 사용자가 방금 누른 그 값으로만 나가게 한다.
    const submittedDayOfWeek = dayOfWeek;
    const submittedTime = `${hour}:00`;
    try {
